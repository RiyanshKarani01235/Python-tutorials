...
void convolution(int *image,int *return_image,int *kernel,int image_width, int image_height, int kernel_size, int kernel_normalizer) {
    ...
    for(i=image_starting_y;i<image_height-kernel_size;i++) {
        for(j=image_starting_x;j<image_width-kernel_size;j++) {
            ...
            *(return_image + (image_width*i) + j) = (sum/kernel_normalizer);
            // return_image[i][j] = sum/kernel_normalizer;
        }
    }
}

// void get_magnitude_and_angle(int *grad_x,int *grad_y,int image_width,\
//     int image_height,int flag) {
...
//     convolution(image,&sobel_x[0][0],&kernel_x[0][0],image_width,\
//         image_height,3,1);
//     convolution(image,&sobel_y[0][0],&kernel_x[0][0],image_width,\
//         image_height,3,1);
